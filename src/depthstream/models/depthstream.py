...
from depthstream.models.aggregator import Aggregator
from depthstream.heads.camera_head import CameraHead
from depthstream.heads.dpt_head import DPTHead
from depthstream.heads.track_head import TrackHead
from transformers.file_utils import ModelOutput
from typing import Optional, Tuple, List, Any, Mapping
from dataclasses import dataclass

@dataclass
class DepthStreamOutput(ModelOutput):
    ress: Optional[List[dict]] = None
    views: Optional[torch.Tensor] = None

class DepthStream(nn.Module, PyTorchModelHubMixin):
    def __init__(self, img_size=518, patch_size=14, embed_dim=1024, depth_condition=None):
        super().__init__()
        ...
        if depth_condition is not None:
            condition_dict = {}
            if '+' in depth_condition:
                condition_array = depth_condition.split('+')
            else:
                condition_array = [depth_condition]

            for condition in condition_array:
                k, v = condition.split('_')
                condition_dict[k] = int(v)
        else:
            condition_dict = None
    ...
    def construct_condition_model(self):
        ...
        if "dino" in self.condition_dict:
            cond_channels = self.condition_dict["dino"]
            self.aggregator.patch_embed.patch_embed.init_alpha_conv(cond_channels=cond_channels)

        if "encoder" in self.condition_dict:
            raise NotImplementedError()
            cond_channels = self.condition_dict["encoder"]
    ...
    def forward(
        ...
        aggregated_tokens_list, patch_start_idx = self.aggregator(images, dino_cond=dino_cond)
        predictions = {}
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            for s in range(S):
                ...
                ress.append(res)
            return DepthStreamOutput(ress=ress, views=views)  # [S] [B, C, H, W]
    ...
    def inference(self, frames, query_points: torch.Tensor = None, past_key_values=None):
        ...
        output = DepthStreamOutput(ress=all_ress, views=processed_frames)
        return output